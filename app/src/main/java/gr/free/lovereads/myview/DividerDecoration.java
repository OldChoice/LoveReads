package gr.free.lovereads.myview;
...
public class DividerDecoration extends RecyclerView.ItemDecoration {
    ...
    public DividerDecoration(Context context, int orientation, int color, int dividerSize, int marginLeft, int marginRight) {
        ...
        mMarginLeft = dip2Px(context, marginLeft);
        mMarginRight = dip2Px(context, marginRight);
     ...
     * @param color       分割线颜色
     */
    public DividerDecoration(Context context, int orientation, int color) {
        if (orientation != LinearLayoutManager.VERTICAL && orientation != LinearLayoutManager.HORIZONTAL) {
            throw new IllegalArgumentException("Orientation is invalidate");
        }

        mOrientation = orientation;
        mDividerSize = dip2Px(context, 1);
        mMarginLeft = dip2Px(context, 0);
        mMarginRight = dip2Px(context, 0);
    ...
    /**
     * 直接默认为横线vertical,颜色为灰色,分割线高度为1，其它不变
     */
    public DividerDecoration(Context context) {
        ...
        mPaint = new Paint(Paint.ANTI_ALIAS_FLAG);//设置为抗锯齿
        mPaint.setColor(Color.parseColor("#f4f4f4"));//设置画笔颜色
        mPaint.setStyle(Paint.Style.FILL);//设置为填充
    }
    ...
    //直接转换复制这个代码直接可用
    int dip2Px(Context context, int dip) {
